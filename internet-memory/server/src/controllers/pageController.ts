...
export const getPages = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (search && apiKey) {
      ...
      const queryVector = await generateEmbedding(search as string, apiKey);
      ...
      const scoredPages = pages.map((page: any) => {
        ...
        if (pageVector && pageVector.length === queryVector.length) {
          ...
          const queryText = (search as string).toLowerCase();
...
export const toggleFavorite = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (isBookmarked !== undefined) page.isBookmarked = isBookmarked;
